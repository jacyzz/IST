...
def match_identifier(root):
    ...
    def find_for_statement_identifier(u, arg, st):
        if u.type == "for_statement":
            arg["in"] = True
        if arg["in"] and u.type == "identifier":
            st.add(u.id)
        for v in u.children:
    ...
    if get_lang() == "c":
        ...
        def check(u):
            if u.type not in ["identifier", "field_identifier"]:
                return False
            if u.parent.type == "function_declarator":
                return False
            if u.parent.type == "call_expression":
                return False
            if u.parent.type == "parameter_declaration":
                parameter_declaration_sons[text(u)] = True
            if text(u) in ["cout", "endl"]:
                return False
            if text(u) in cpp_keywords:
                return False
            if u.parent.type == "pointer_declarator":
                return True
            if (
                u.parent.type == "field_expression"
                and len(text(parent(u, 1)).split("->")) == 2
                and text(u) == text(parent(u, 1)).split("->")[0]
            ):
                return True
            if u.parent.type in [
                "function_declarator",
                ...
                return False
            if u.id in for_statement_identifiers_ids:
                return False
            if len(text(u)) == 0:
                return False
            if get_lang() == "java" and u.parent.type in ["scoped_identifier"]:
                return False
            return True
        ...
        def check(u):
            if u.type not in ["identifier"]:
                return False
            if u.parent.type in ["member_access_expression"]:
                return False
            return True
...
def is_all_lowercase(name):  # aaabbb
    return (
        name.lower() == name
        and not is_snake(name)
        and not is_init_underscore(name)
        and not is_init_dollar(name)
    )
...
def sub_token(name):  # Turn a token into a subtoken
    ...
    if is_camel(name):
        ...
        if len(tokens) == 1:
            rp = len(tokens[0]) // 2
            tokens = [tokens[0][:rp], tokens[0][rp:]]
...
def count_pascal(root):
    ...
    res = 0
    for node in nodes:
        res += is_pascal(text(node)) and node.parent.type not in [
            "scoped_identifier",
            "method_declaration",
            "field_access",
            "method_invocation",
...
def convert_hungarian(node):  # intAabb
    ...
    if lang == "c":
        ...
        def find_type(u):
            if len(_type) > 0:
                return
            if text(u) == text(node) and u.parent.type in [
                "declaration",
                "init_declarator",
            ]:
